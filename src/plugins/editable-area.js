...
function setup(cm) {
  ...
  try {
    cm.doc.eachLine((line) => {
      const lineText = cm.lineInfo(line).text;
      const lineMatches = lineText.matchAll(new RegExp(regex, "g"));
      ...
      for (let res of lineMatches) {
        ...
        let tagType = "tag";
        const multiline = tagsInfos[res[1]]?.multiline || false;
        ...
        if (res[1] in tags) {
          ...
          throw "configError";
        } else if (multiline && lineText !== res[0]) {
          throw "configError";
        }
    ...
    if (e === "configError") {
      cm.setValue("Configuration error. please check the config");
      cm.doc.clearHistory();
...
function onCursorActivity(cm) {
  if (cm.doc.somethingSelected() || !cm.hasFocus()) return;
  const cursorPos = cm.doc.getCursor();
